...
class ClientThread(Thread):
                ...
                # -----------------------------------------------------------------------------------------------
                    ...
                    if alert_row_id and dev_sl and dev_name and dev_alert_type and alert_number:
                        if dev_alert_cause == 'motion' and is_motion_found:
                            ...
                            self.set_all_previous_unsnooz_entries_to_snooze(dev_sl)
                            # -----------------------------------------------------------------------------------------------
    ...
    # ***********************************************************************************************
    def set_all_previous_unsnooz_entries_to_snooze(self, dev_sl):
        # -----------------------------------------------------------------------------------------------
        cur, conn = False, ''
        # -----------------------------------------------------------------------------------------------
        try:
            ...
            AND is_alert_snooze = ?             
            AND id NOT IN (SELECT id FROM app_devices_motionvideofromdevice WHERE device_info_id_id = (SELECT id FROM app_devices_deviceinfo WHERE dev_sl = ?) ORDER BY id DESC LIMIT 1)
            """, (True, dev_sl, False, dev_sl))
            # -----------------------------------------------------------------------------------------------
            conn.commit()
            print('set_all_previous_unsnooz_entries_to_snooze SUCCESS!!!!!!!', dev_sl)
        # -----------------------------------------------------------------------------------------------
        except Exception as e:
