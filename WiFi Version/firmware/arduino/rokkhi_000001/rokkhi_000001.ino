...
void loop()
{
  ...
  if (!client.connected())
  {
    ...
    // ----------------------------------------------------------------------------------------------------
    if (cloud_connection_lost_cnt > 10)  // Check for continuous 10 Secs
    {
      if (DEBUG)
      {
  ...
  else
  {
    ...
    if (DEBUG)
    {
      Serial.println("Cloud connection is ALIVE");
    }
  }
  // ====================================================================================================
    ...
    while (detectMotionByPIR())
    {
      ...
      if (server_confirmed_start_img_flag)
      {
        ...
        while (detectMotionByPIR() && (total_no_of_frames < no_of_frame_capture_limit))
        {
          ...
          {
            client.write("dAtAsTaRt", 9);             // Send Start marker before sending image
            delay(1);
            // client.write((uint8_t*)&fb->len, 4);   // Send Size
            client.write(fb->buf, fb->len);           // Send Data
            esp_camera_fb_return(fb);
            delay(1);
            client.write("eNddAtA", 7);               // Send End marker before sending image
            delay(1);
          }
          // ----------------------------------------------------------------------------------------------------
